...
program_name <- "arcasHLA"

hisat_index_path_file <- "/media/hugolab/DataRAID1/projects/annotations/hisat2/grch38_snp_tran/genome_index.txt"
allocated_thread_number <- 12
```

# input and output

```{r}
local_program_directory <- file.path(getwd(), "output_directory", "local_directory", program_name) %T>%
    dir.create(., showWarnings = F, recursive = T)

unlink(file.path(local_program_directory, "*"), recursive = T)

all_inclusive_table_path <- file.path(getwd(), "output_directory", "entity_samples_table.tsv")
...
for (i in seq_len(nrow(all_inclusive_dataframe))) {
    ...
    current_sample_name <- all_inclusive_dataframe[i, "base_file_name"]
    
    new_boilerplate_list <- raw_boilerplate_list
    
    new_boilerplate_list$MyBestWorkflow.base_file_name <- all_inclusive_dataframe[i, "base_file_name"]
